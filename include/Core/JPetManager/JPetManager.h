...
public:
  ...
  /**
   * @brief Method parses command line arguments and returns
   * the set of validated options generated based on it.
   *
  ...
  /**
   * @brief Method to add the task to the chain of tasks
   * that will be executed later by JPetTaskExecutor.
   ...
   * e.g. if inputFileType is "raw" and file name is "bla", then
   * the input file name is expected to be "bla.raw.root".
   * There are some labels that are treated separately e.g. "zip" or "hld".
   * If the outputFileType is the empty string then the task is assumed to have no output tree.
  ...
  void useTask(
    ...
    const std::string& outputFileType = "",
    int numTimes = 1
  );
