...
public:
   ...
   * Warning(!): the added tasks are wrapped in the special tasks that handle the I/O operations,
   * so effectively they become subtasks of the I/O tasks.
   * @param options set of options that are passed to decide which
   * default tasks will be added at the beginning of the chain e.g. unzipping task or unpacking task.
...
private:
  ...
  /**
   * Map containing the registered task types.
   */
  std::map<std::string, TaskGenerator> fTasksDictionary;

  /**
   * The collection of task information
   * (name, input and output file type) corresponding to task chain to be created.
   */
  std::vector<TaskInfo> fTasksToUse;
};
 ...
 * @param taskInfoVect vector of task information
 *  which will be transform into the chain of the corresponding generators
 * @param generatorMap map of registered tasks. Only those task can be used to produce the task generators
 * @param options set of options that are passed to decide which default tasks
 ...
 * @param generatorMap map of registered tasks.
 *  Only those task can be used to produce the task generators.
